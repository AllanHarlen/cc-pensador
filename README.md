...
- `design-systems/<id>/` — Verbatim Open Design system files (`tokens.css`, `DESIGN.md`, `components.html`, `preview/`, `system/`, `source/`, …), fetched by `scripts/od-fetch-system.mjs` into the feature folder, then checked for DESIGN.md-vs-tokens.css content divergence by `scripts/od-verify-system.mjs` (writes `design-consistency.json`, never rewrites DESIGN.md). *(both modes, when front-end exists and a system was selected; the Executor materializes them into `packages/ui`/`src/styles` during implementation)*
