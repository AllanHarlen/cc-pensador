...
- `integrations.openDesign` (opcional, condicional a front-end): se a demanda tiver front-end e o Open Design (`od`) nao for detectado, ofereca instalacao via `AskUserQuestion` rodando o script `scripts/install-open-design.ps1` (Windows) ou `scripts/install-open-design.sh` (macOS/Linux), que clona e compila o Open Design, sobe o daemon NO HOST (nao em Docker) e conecta o MCP; ou caia para `design-system.md` inline. Com o Open Design no ar, puxe o DESIGN.md via `od design-systems list/show` (ou pela API do daemon em `http://127.0.0.1:7456`). Veja `skills/pensador/references/open-design.md`.

- `integrations.designAgents` (mandatório sempre que o Open Design for usado, com front-end): agentes que podem gerar o protótipo/Critique do Open Design (`agents[]` com `id`, `where` = `host`, `available`, `authenticated`; `daemonWhere` = `host`, ou `container` só quando um container Docker legado segura a porta, o que é recusado). Com front-end e o Open Design em uso, pergunte **uma vez** via `AskUserQuestion` (`header: "AgenteDesign"`, opções = agentes detectados; **a opção de pular não é oferecida**, a escolha é obrigatória) e grave com `scripts/design-brief.mjs agent --agents <preflight.json> --choose <id>` (recusa `--choose none`). Sem nenhum agente `available`, **bloqueie**: nao ha como fechar o DESIGN sem prototipo quando o Open Design e usado — ofereca instalar/autenticar um agente (mesma remediacao de `agent-not-visible-to-daemon`) antes de seguir. Com o agente vinculado, `od project create` + `od run start --agent <id>` **so depois do aceite explicito do usuario em pergunta separada** (o run consome tokens), exportando os arquivos gerados para `<featurePath>/prototypes/` (role `design-prototype`). Veja `skills/pensador/references/open-design.md`.
...
| `scripts/install-open-design.ps1` / `scripts/install-open-design.sh` | Instalador opcional do Open Design no host (verifica git/node/corepack, compila o clone, sobe o daemon no host com os agentes no PATH, conecta o MCP), oferecido via `AskUserQuestion` quando ha front-end |
| `scripts/onboard-open-design-agents.ps1` / `scripts/onboard-open-design-agents.sh` | Registra os agentes do host (claude/codex/antigravity), compila e sobe o daemon no host; guarda de porta contra um container Docker legado (`-StopLegacyContainer`); `-Foreground` para supervisores |
| `scripts/register-open-design-daemon-task.ps1` | Windows: Tarefa Agendada por usuario que sobe o daemon do host a cada logon e o reinicia se cair (`-StartNow`, `-Unregister`) |
| `scripts/od-mcp-config.mjs` | Helper que busca `/api/mcp/install-info` do daemon e faz merge da entrada `mcpServers.<nome>` no `.mcp.json` (usado pelo instalador quando nao ha `od` no PATH) |
