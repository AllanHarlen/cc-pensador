...
export function validateVisualCompleteness(handoff, options = {}) {
  ...
  // that case (see resolveDesignAgent/agentCommand, requireAgent). The
  // inline fallback (designFallback, no Open Design) never requires it.
  if (designFiles && !byRole("design-prototype")) {
    push(
      "MISSING_DESIGN_PROTOTYPE_FOR_DONE_STATUS",
      "status DONE with an Open Design package (design-system-files) requires a design-prototype artifact (interactive HTML/CSS/JS prototype generated by the chosen design agent in the DESIGN stage) — see references/open-design.md.",
      "artifacts[]",
    );
  }
