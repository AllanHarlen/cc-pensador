...
2. **Prototipacao no Discovery:** Selecione os 1 a 3 fluxos criticos descritos no `PRD_Base` (ex.: Vitrine + Carrinho, Dashboard de Metricas, Onboarding) e instrua a geracao de prototipos HTML standalone interativos em `<featurePath>/prototypes/<fluxo>/index.html`. Os prototipos devem consumir o CSS local (`tokens.css`), ser autocontidos (sem dependencias de CDNs externas) e acessiveis (WCAG AA).
   - **Fidelidade de superficie `conversion`/`catalog` (obrigatorio quando `state.surfaces` tiver uma):** o protototipo dessa superficie precisa cobrir toda secao de `surface-benchmark.json` (`baselineSections` + `benchmarkedSections`) que o usuario nao recusou explicitamente no passo 8 do RESEARCH, com imagens reais (nao placeholder) e responsivo. Um wireframe minimo — hero + formulario, sem prova social/diferenciais/contato — nao e spec autoritativa para essa superficie: uma run real (OficinaAI, 2026-09-16) aprovou um protótipo assim, o Orquestrador tratou como "SPEC VISUAL imperativa" e a vitrine publica entregue ficou sem nenhum dos elementos que os concorrentes reais usam. Confira a cobertura contra o benchmark ANTES de apresentar ao usuario no passo 3.
