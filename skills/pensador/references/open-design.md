...
| DESIGN — contrato e render | `design-contract.json` (v2) → `tokens.css`, `components.css`, `design-tokens.json` (DTCG), `tailwind-v4.css`, `DESIGN.md`, `components.html`, `preview/`, `USAGE.md`, `manifest.json` (`design-package.mjs render`, obrigatório) |
| DESIGN — gates | `design-package.mjs audit` → `design-audit.json` com `checks` por gate: `structure` (arquivos, escalas monótonas, estados com `focus-visible`), `contrast` (matriz WCAG 2.2 AA nos temas claro **e** escuro, só variantes seguras), `conformance` (campo **travado** do brief que diverge bloqueia; primária comparada só ao tema claro, ΔE ≤ 2), `integrity` (re-render em memória byte a byte + `provenance.json`) e `engineRun` (`source/engine-run.json` com `status: "ok"` para o mesmo contrato); mais `componentCoverage` (`PASS`/`WARN`, não bloqueante: componente do contrato sem regra no `components.css`). A review read-only (Codex) é registrada por `design-package.mjs review --verdict PASS|FAIL --reviewer codex` em `design-review.json`, amarrada ao `contractSha256`; o gate do DESIGN exige review `PASS` do contrato atual (`DESIGN_REVIEW_MISSING`/`_STALE`/`_NOT_PASS`) |
...
Regra: **o daemon só lança um agente que existe no ambiente dele.** Um agente que o daemon do host não enxerga devolve `agent-not-visible-to-daemon` com a remediação `install-and-authenticate-<id>-in-daemon-host`; um daemon que na verdade é um **container legado** devolve a mesma recusa com `stop-legacy-container` e `start-host-daemon` (o container não vê nenhum agente do host). O Pensador pergunta como proceder via `AskUserQuestion`. **Sem nenhum agente `available`** (nenhum detectado no host nem visível ao daemon), o protótipo não pode ser gerado, mas ele é obrigatório sempre que o Open Design é usado com front-end: o Pensador não segue em silêncio — pergunta via `AskUserQuestion` como instalar/autenticar um agente e não fecha o DESIGN enquanto isso não for resolvido.

Pergunta (uma vez, antes do DESIGN, com front-end e o Open Design em uso; `multiSelect: false`, `header: "AgenteDesign"`):

> **Qual agente deve gerar o protótipo do design e a crítica (Open Design)?** — uma opção por agente detectado (descrição `id@where`); a seleção é obrigatória, a opção de pular não é oferecida.

A escolha é gravada por `design-brief.mjs agent --agents <preflight.json> --choose <id>` em `state.designAgent` (`{ id, where, chosenAt }`; validado por `validateDesignAgent`; `--choose none` é recusado). O hook `track-questions.mjs` registra só o cabeçalho da pergunta. Escolher o agente ainda não dispara o run: `od run start` consome tokens e só roda após uma **segunda pergunta** de aceite explícito do usuário (ver a seção seguinte). O protótipo é gerado via `od project create --design-system user:<id>` e `od run start --agent <id>` com o design system registrado no daemon, e os arquivos de página/interface gerados são exportados para `<featurePath>/prototypes/` (role `design-prototype` no handoff), servindo de referência de fidelidade visual para as tasks de front-end do Orquestrador reproduzirem na stack de front-end definida pelo PRD — não um artefato copiado ou convertido mecanicamente.

### Registro do design system no daemon
...
2. Sobrepõe `tokens.css`, `design-tokens.json`, `tailwind-v4.css`, `components.css`, `components.html`, `components.manifest.json`, `USAGE.md`, `manifest.json`, `DESIGN.md` e `preview/` nesse diretório, e `tokens.css` também sobre `colors_and_type.css` (o wrapper não entrega uma paleta genérica por outro canal). O `--data-dir` padrão é o do daemon do host (`~/.open-design/.od`, ou `OD_DATA_DIR`); o daemon em Docker (`--container`) não é mais suportado.
...
**Limite honesto:** o passo 2 escreve em um diretório que o daemon possui. Isso **não é API pública** e foi validado só no daemon **0.22.1**; a versão (`/api/health`) fica em `statePatch.designRegistrations[<id>].daemonVersion` e a ausência do diretório esperado é a recusa `OD_REGISTER_LAYOUT_MISSING` com remediação (apontar o `--data-dir` certo ou usar uma versão validada), nunca um registro parcial silencioso. O daemon do host leva de 10 a 30 s para responder no primeiro start. O script **nunca** dispara `od run start`: o run consome tokens, exige o aceite explícito e usa o agente escolhido em `AgenteDesign` (`od run start --daemon-url <url> --project <id> --agent <designAgent.id>`). O protótipo é obrigatório sempre que o Open Design é usado com front-end (a escolha do agente na seção anterior não tem opção de pular), mas continua **uma execução de um agente não determinístico**: a fidelidade do resultado final é garantida pelos gates do Orquestrador/Executor e pelo probe do Testador, não pelo protótipo em si (evidência: subseção 10.10.2 do plano, 23 de 23 tokens em cada tema).
