...
>
> ⚠️ **`od-fetch-system.mjs` valida presença, não conteúdo — `DESIGN.md` e `tokens.css` podem divergir entre si mesmo copiados corretamente.** Os dois são gerados por upstream no mesmo bundle, mas nada garante que descrevem o mesmo produto: uma run real (2026-09-05) teve `DESIGN.md` prometendo primária amarela (`#FECE14`) e tipografia Poppins/IBM Plex Mono, enquanto `tokens.css` — a fonte de verdade, que a implementação seguiu corretamente — definia `--accent: #2563eb` (azul) e Inter/SF Mono; `source/tokens.source.json` listava `DESIGN.md` entre os arquivos-fonte sem nenhum dos 56 tokens de fato traçar até lá. `DESIGN.md` é o artefato que um humano lê para julgar conformidade — e descrevia um produto que nunca existiu. Rode `od-verify-system.mjs --dir <featurePath>/design-systems/<id>` (SKILL.md, passo 5, item 4) logo após o fetch: compara hex de cor, família tipográfica e escala de espaçamento entre a prosa e os tokens, grava `design-consistency.json`, e nunca tenta regenerar o `DESIGN.md` a partir do `tokens.css` — o bundle é cópia verbatim, reescrever quebraria a proveniência.
