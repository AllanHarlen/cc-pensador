...
**Context7 (`integrations.context7` do preflight), quando disponivel, e a fonte preferida desta fase** — documentacao oficial versionada em vez de resultado de busca generico, coerente com a inversao de tiers de fonte da secao seguinte. Ordem obrigatoria, sem atalho: **resolver o identificador da biblioteca (nome livre -> identificador do Context7) antes de pedir a documentacao**, mesmo quando o identificador "ja e conhecido" de uma consulta anterior na mesma feature — nome de pacote e identificador do servidor nao sao a mesma coisa, e pedir documentacao de um identificador inventado devolve conteudo errado ou vazio. Regras operacionais do Context7: (1) use o nome oficial pontuado da biblioteca ao resolver o ID (ex: `Next.js`, `ASP.NET Core`, `Three.js`); (2) quando a stack detectada fixa versao (manifest do projeto, em brownfield), selecione preferencialmente o identificador versionado retornado em `Versions` no formato `/org/project/version` (ex: `/vercel/next.js/v14.2.0`); (3) faca chamadas `query-docs` focadas em um unico conceito por vez (Single-Concept Scoping, evitando consultas compostas que diluem o ranking semantico); (4) limite de no maximo 3 consultas ao Context7 por tecnologia. Sem Context7 disponivel, a fase segue via `WebSearch`/`WebFetch` normalmente — a ausencia nunca bloqueia o RESEARCH.
