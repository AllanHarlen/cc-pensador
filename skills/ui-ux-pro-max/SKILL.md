...
Quando `hasFrontend = true`, as lacunas de experiência (especialmente estados de tela, acessibilidade, hierarquia visual e microcopy) compõem o **brief de design** que o Pensador parseia e entrega ao **Open Design** (`od`, MCP/CLI), que seleciona um system cujo `DESIGN.md` (9 seções) é persistido verbatim. Esta lente define *o que* a experiência exige; o Open Design materializa o *sistema de design* correspondente. Se o Open Design não estiver instalado, o Pensador oferece a instalação via `AskUserQuestion` ou escreve um `design-system.md` inline a partir do mesmo schema de 9 seções. Veja `skills/pensador/references/open-design.md`.
