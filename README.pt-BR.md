...
> **Protótipo obrigatório, CSS de componentes e review do design registrada (2.38.0):** sempre que o Open Design é usado com front-end, a escolha do agente do design não tem opção de pular (`design-brief.mjs agent` recusa `--choose none`) e o protótipo interativo vai no handoff como role `design-prototype` (`prototypes/`); handoff DONE sem ele falha em `MISSING_DESIGN_PROTOTYPE_FOR_DONE_STATUS`. O Open Design só gera HTML/CSS/JS: o Orquestrador usa o protótipo como referência de fidelidade visual para as tasks de front-end na stack do PRD, nunca uma conversão mecânica. O render agora entrega `components.css` — regras de produto para ~30 tipos de componente (um contrato real tinha 18 de 24 componentes como placeholder cinza com audit PASS), reportadas pelo `checks.componentCoverage` não bloqueante — e a review do Codex sobre o pacote é registrada com `design-package.mjs review` em `design-review.json`, amarrada ao hash do contrato: o gate do DESIGN exige review PASS do contrato atual e o handoff só declara `validation.status: PASS` com audit **e** review aprovados. O `requirements.json` também indexa requisitos não funcionais (`RNF-XX`) e padrões de arquitetura (`ARC-XX` sintéticos), que o Orquestrador passa a exigir com task e evidência, e o comando `validate` do contrato de API passa a ser executável (`st run openapi.yaml --url <base-url>`).

> **Handoff de design (2.32.0):** o contrato visual em `references/handoff-contract.md` (secao 6, byte-identica nos quatro plugins do workflow) passa a ter `design-systems/<id>/resolved/` como unico pacote normativo; `source/` guarda a proveniencia do engine. A entrada do handoff carrega `contractSha256`, `themes` e `designBriefPath`; token novo so entra por nova versao do Pensador (`DESIGN_CHANGE_REQUEST`).

> **Open Design no host (2.37.0):** o daemon do Open Design agora roda **no host, nunca em Docker** — um container não lança o claude/codex/agy do host, então o agente do design (`AgenteDesign`) nunca conseguiria rodar. Foram removidos o caminho de instalação Docker, `od-brand-build.mjs --engine container`, `OD_CONTAINER` e `od-register-system.mjs --container` (o brand engine e a proposta por URL da marca rodam a partir do clone do daemon do host). O `preflight.mjs` ganhou uma guarda de conflito de porta (`LEGACY_CONTAINER_DAEMON` + `portConflict`), o token vem de `OD_API_TOKEN` ou de `<clone>/.env` (o daemon do host em loopback não exige), todos os clientes de API usam `127.0.0.1` (o daemon responde 403 em `localhost`) e `scripts/register-open-design-daemon-task.ps1` registra uma Tarefa Agendada do Windows que mantém o daemon de pé após reiniciar. Veja o `CHANGELOG.md` para os passos de migração.
>
> **Registro do design system (2.36.0):** o protótipo do Open Design (obrigatório sempre que o Open Design é usado com front-end desde a 2.38.0, entregue no handoff como role `design-prototype`) só respeita os tokens quando o daemon serve o `tokens.css` do `resolved/` verbatim. O `od design-systems import-local` o regenera (0 de 16 tokens escuros idênticos), então o `scripts/od-register-system.mjs` registra o pacote no lugar (`POST /api/design-systems` + sobreposição do `resolved/` + leitura de volta PELO DAEMON) e recusa (`OD_REGISTER_TOKENS_DIVERGED`) se o `tokens.css` servido não for byte a byte o do `resolved/`. Exige o aceite do usuário (`header: "RegistroOD"`), nunca dispara um run, registra a versão do daemon e recusa com remediação quando o layout do daemon não é o validado (0.22.1; a cópia não é API pública).
>
> **Agente do design (2.35.0):** o protótipo/Critique do Open Design (obrigatório com Open Design desde a 2.38.0) precisa de um agente de código vinculado ao daemon. O `preflight.mjs` reporta `integrations.designAgents` (PATH do host + o que o daemon do OD enxerga via `/api/agents`, com `daemonWhere`); com front-end e ao menos um agente disponível o Pensador pergunta uma vez via AskUserQuestion (`header: "AgenteDesign"`, opções = agentes detectados; desde a 2.38.0 não há opção de pular e a ausência de agente bloqueia o estágio), grava `state.designAgent` (`design-brief.mjs agent`) e usa `--agent <id>` no `od run start` só após o aceite explícito (consome tokens). O daemon roda no host (Docker não é suportado: um container não lança os agentes do host); por isso um agente que o daemon não enxerga é recusado com uma remediação (instalar/autenticar no host), e um container Docker legado segurando a porta do daemon também é recusado (`LEGACY_CONTAINER_DAEMON`, com os passos `docker stop` / `docker update --restart=no` / subir o daemon do host).
>
> **Aprovação do design (2.34.0):** a assinatura da aprovação é um HMAC-SHA256 feito com uma chave por usuário guardada fora do workspace (`%LOCALAPPDATA%/pensador/approval.key` ou `~/.pensador/approval.key`, criada no primeiro `approve`, nunca impressa); sem a chave o gate falha com `DESIGN_APPROVAL_KEY_MISSING` (aprove de novo nessa máquina). O guard bloqueia Read/Grep/Edit/Write/shell sobre a chave. Limite: quem executa código arbitrário como o mesmo usuário do SO lê a chave. Desde a 2.33.0 a aprovação visual é assinada: `design-brief.mjs approve` exige a pergunta `AprovDesign` do AskUserQuestion registrada pelo hook e grava `.pensador-approval.json`; o gate rejeita aprovação digitada à mão e o guard bloqueia escrita direta no brief e no registro. O campo opcional `brandUrl` (`design-brief.mjs brand-url`) deixa o engine propor cor primária e fonte a partir do site da marca, confirmadas pelo usuário. `test/design-e2e-real.test.js` roda o engine real de ponta a ponta.

`versão 2.37.0` · `categoria: planning` · todo diálogo passa **exclusivamente** por `AskUserQuestion`.
...
| **DESIGN** | Materializa o pacote de design resolvido (`design-systems/<id>/resolved/`): contrato de design, tokens, `components.css` (regras de produto dos componentes), `components.html`, previews, brand assets auditados; registra a review do Codex (`design-review.json`) e, com Open Design, exporta o protótipo gerado pelo agente para `prototypes/`. Auto-avança em demanda back-end-only. | AGY (síntese + imagens) · Codex (auditoria read-only) | exceto back-end-only |
| **FINAL** | Aplica `withConsolidated`, confirma back-end, gera artefatos (`requirements.json` com RF/CA, RNF e ARC no modo PRD), valida o handoff, apresenta recap e handoff. | — | sempre |
...
- `design-system.md` — Sistema de design brand-grade (schema DESIGN.md), escrito inline **apenas como fallback** quando o Open Design não está disponível. Quando o Open Design é usado, o `DESIGN.md` gerado (em `design-systems/<id>/resolved/`) é o documento de design e nenhum arquivo standalone redundante é gerado. *(modo PRD, quando houver front-end)*
- `design-systems/<id>/` — O design system gerado: `source/` (proveniência do engine: `brand.json`, saída bruta, `engine-run.json`) e `resolved/` (`design-contract.json` v2, `tokens.css` com temas claro e escuro, `design-tokens.json` DTCG, `tailwind-v4.css`, `components.css` (a única folha de componentes que o app importa), `DESIGN.md`, `components.html`, `preview/`, `design-audit.json`, …). Os tokens são derivados pelo brand engine do Open Design (`scripts/od-brand-build.mjs`: clone local do daemon do host → `BLOCKED`), nunca escritos por LLM; `scripts/design-package.mjs render` gera todos os arquivos a partir do contrato e os audita. `<id>` é derivado do produto, nunca um id de catálogo. *(nos dois modos, quando houver front-end; o handoff registra o status real da auditoria e o `contractSha256`, e o Executor materializa em `<uiPackageDir>/design-systems/<id>/` na implementação)*
- `design-brief.json` — O brief de design como contrato: cada resposta é um campo `{ value, locked, questionRef }` (campo travado sempre prevalece sobre proposta), mais a aprovação visual (`approvedAt`, `approvedSha256`). Gravado por `scripts/design-brief.mjs` (`build`, `seed`, `adjust`, `approve`). O estágio DESIGN só fecha quando o `design-audit.json` passa em cinco checks (estrutura, contraste WCAG nos **dois** temas, conformidade com o brief, integridade do render, engine run) e o usuário aprovou o preview **deste** contrato. *(quando houver front-end)*
- `prototypes/` — Protótipo interativo HTML/CSS/JS gerado pelo agente do Open Design escolhido (role `design-prototype`). **Obrigatório sempre que o Open Design é usado com front-end**: a escolha do agente não tem opção de pular e um handoff DONE sem ele falha em `MISSING_DESIGN_PROTOTYPE_FOR_DONE_STATUS`. É referência de fidelidade visual que o Orquestrador entrega às tasks de front-end para reproduzir na stack do PRD — nunca convertido mecanicamente.
- `requirements.json` — Índice determinístico do PRD: `requirements` (RF) e `acceptanceCriteria` (CA, ligados a RF), mais `nonFunctionalRequirements` (RNF, seção 7) e `architecturePatterns` (ids ARC sintéticos, seção 15). O Orquestrador exige task e evidência para cada id; o Testador monta sua matriz de cobertura a partir dele. *(modo PRD)*
...
└── <slug-da-demanda>-vN/          ← ex.: login-social-v1
    ...
    ├── design-systems/<id>/       ← design system gerado (source/ + resolved/: tokens.css, components.css, DESIGN.md, components.html, preview/, …)
    └── prototypes/                ← protótipo do Open Design (role design-prototype; obrigatório com Open Design)
```
...
- Relevante apenas quando a demanda tem **front-end** (`hasFrontend`). No **BRAINSTORM_GERAL**, o Pensador parseia um **brief de design** via `AskUserQuestion` — setor, tom visual, marca/referências, paleta de cores, tipografia, estados de componente, responsividade, acessibilidade (alvo WCAG), microcopy, tema padrão e exposição do tema no app. Cada resposta vira um campo `{ value, locked, questionRef }` do `design-brief.json` (`buildDesignBrief()`); campos travados sempre prevalecem sobre qualquer proposta, e `briefToSeed()` converte o brief no seed do brand engine.
- No **DESIGN**, esse brief vira o seed do brand engine do Open Design e o Pensador gera o design system em `<featurePath>/design-systems/<id>/` — **não há catálogo**: o Pensador nunca lista, baixa nem verifica systems prontos do Open Design. O `resolved/DESIGN.md` gerado (paleta, tipografia, espaçamento, layout, componentes, motion, voz, marca, anti-padrões — 9 seções) **é** a decisão de design; não há reescrita em `design-system.md` standalone quando o Open Design é usado.
- Com o Open Design em uso, o **protótipo é obrigatório** (2.38.0): o Pensador vincula um agente do design (sem opção de pular) e — após um aceite de custo separado — roda `od run start` contra o design system registrado e exporta as páginas para `<featurePath>/prototypes/` (role `design-prototype`). A review do Codex sobre o pacote resolvido é registrada em `design-review.json` para o hash do contrato atual; sem review PASS o DESIGN não fecha.
...
Instalação: o Open Design é um app local-first (daemon + web); não há `curl | sh` de uma linha. O cc-pensador traz um script instalador que roda o daemon **no host, não em Docker** — `scripts/install-open-design.ps1` (Windows) / `scripts/install-open-design.sh` (macOS/Linux): verifica git/node/corepack, compila o clone, registra claude/codex/antigravity no daemon, sobe-o (app em http://127.0.0.1:7456, sem token de API em loopback) e conecta o MCP via `od mcp install <agent>`. No Windows, `scripts/register-open-design-daemon-task.ps1` (ou `-Autostart` no instalador) registra uma Tarefa Agendada por usuário que sobe o daemon a cada logon e o reinicia se cair. Use `127.0.0.1`, nunca `localhost` (o daemon responde 403 a clientes de API em `localhost`). O Pensador oferece rodar o instalador via `AskUserQuestion` quando a demanda tem front-end. Veja `skills/pensador/references/open-design.md`.
